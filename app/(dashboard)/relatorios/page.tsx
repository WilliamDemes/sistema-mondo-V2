  ...
  return (
    <div className={styles.rp}>
      ...
      <div className={styles["rp-header"]}>
        <div><h1 className={styles["rp-title"]}>Relatórios</h1><p className={styles["rp-sub"]}>Exporte informações consolidadas das famílias</p></div>
        <div className={styles["rp-export"]}>
          <button className={styles["rp-btn-pdf"]} onClick={exportPDF} disabled={loadingDetails||familyDetails.length===0}><Printer size={16}/>Exportar PDF</button>
          <button className={styles["rp-btn-excel"]} onClick={exportExcel} disabled={loadingDetails||familyDetails.length===0}><FileSpreadsheet size={16}/>Exportar Excel</button>
        </div>
      </div>
      ...
      <div className={styles["rp-config"]}>
        <div className={styles["rp-col"]}>
          ...
          <div className={styles["rp-fields"]}>
            {REPORT_FIELDS.map(f=>(
              <label key={f.key} className={`${styles["rp-check"]} ${selectedFields.has(f.key)?styles["rp-check-active"]:""}`}>
                <div className={`${styles.ckbox} ${selectedFields.has(f.key)?styles.ckd:""}`} onClick={()=>toggleField(f.key)}>{selectedFields.has(f.key)&&<Check size={14}/>}</div>
                <div><span className={styles["ck-l"]}>{f.label}</span><span className={styles["ck-d"]}>{f.desc}</span></div>
              </label>
            ))}
          </div>
        </div>
